...
\subsection{Service-layer Implementation for Tiered Access Control}
...
Django is a credible option for a feasibility demonstrator of the White Book access model because it already provides:
\begin{itemize}
...
If the immediate goal is to verify that the public rendering tier, the private computation tier, and the restricted expert tier for parameter editing, calibration, and Bayesian inference defined in Chapter~\ref{chap-arch-decomp} can be enforced reliably, Django is therefore appropriate as a \textbf{service-boundary} technology, \ie for authentication, authorization, entitlement management, and request routing.
...
This service-layer orientation complements the rest of the present chapter, which documents how the underlying \hydrot\ backend remains the scientific core reused by \cwv.
