...
\begin{itemize}
    ...
    \item \texttt{optimization\_algorithm}: Selection of optimization method (\texttt{'gradient\_descent'}, \texttt{'mcmc'}, \texttt{'dream'}, \texttt{'enkf'}, \texttt{'pso'}, \texttt{'multi\_stage\_dream'})
    ...
    \item \texttt{algorithm\_kwargs}: Algorithm-specific parameters (learning rate, ensemble size, DREAM chains, workflow stages, etc.)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{DREAM integration}: \texttt{EstimationLayer.run\_dream()} for multi-chain MCMC via Molonari.io
    \item \textbf{HYMIT integration}: \texttt{EstimationLayer.run\_hymit()} for frequency-domain parameter identification
\end{itemize}
