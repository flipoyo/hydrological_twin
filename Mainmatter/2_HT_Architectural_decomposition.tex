...
For an implementation that follows the White Book access-control cases efficiently, the safest convention is a \textbf{fail-closed} one: the authenticated \texttt{HydroTwin} service class should be declared \emph{private by default}, and only the explicitly approved public methods should be marked as public.
