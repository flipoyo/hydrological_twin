...
\end{itemize}
Public users interact with a \textbf{restricted state-space subset}, limited to aggregated and  masked variables.
...
\subsection{The \texttt{private-only} Components: Instantiation, Physical Model Parameterizations and Extraction from Simulation Hyper-Cube}
\label{sec:private-components}

By design, HydrologicalTwin isolates \textbf{three categories of private components}:
\begin{enumerate}
    \item \textbf{Instantiation}: Creating a new \hydrot\ instance requires private-key authentication to ensure proper provenance tracking and accountability. Instantiation establishes the git-synchronized state of the digital twin and appends it to the universal registry with cryptographic traceability.
    ...
    \item \textbf{Data extraction and analysis operations}: Raw simulation data (e.g., full-resolution NumPy hypercubes) and advanced analytical operations (e.g., \texttt{extract-values}, \texttt{self-fitting}, \texttt{frequency-domain-analysis}, \texttt{bayesian-inversion}) require \textbf{private-key authentication}. Without a valid key, users are limited to:
    \begin{itemize}
        \item Masked and aggregated outputs (e.g., basin-level averages, \texttt{apply-temporal-operator}, \texttt{apply-spatial-operator}, both in upscaling mode upper than the simulation resolution).
        \item Visualization layers (e.g., spatial maps, time-series plots) via the \texttt{Rendering} layer.
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \hydrot\ instantiation
        \item Raw data extraction.
        \item Parameter modification.
        \item Execution of new simulations (forward model execution)
        \item Uncertainty based on bayesian framework
        \item AI computing
    \end{itemize}
