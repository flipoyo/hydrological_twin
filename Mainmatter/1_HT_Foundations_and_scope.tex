...
Traceability is not an optional feature but a structural invariant: every \hydrot\ object carries a Git-aware identity card embedding commit hashes, branch or tag identifiers, dirty state, and remote URLs. The universal registry further anchors each validated digital twin instance in an append-only, hash-chained ledger synchronized on Git.
...
\subsection{The Only Private Component: Physical Model Parameterizations}

By design, the only component of a \hydrot\ instance that may remain private is the parameterization of physically--based models.

Model parameters often encode strategic, economic, or intellectual capital, especially in operational or industrial contexts. For this reason, \hydrot\ explicitly distinguishes between:
\begin{itemize}
\item public artefacts: code, datasets, forcings, spatial supports, observational products, and simulation outputs,
\item private artefacts: internal parameter sets of physically--based models.
\end{itemize}

This separation is enforced at the repository level through Git access policies. The parameter repositories can remain private, while their identity cards and cryptographic fingerprints are still registered in the space--state and in the universal registry.

As a result, it becomes possible to prove the existence, version, and lineage of a given parameterization without revealing its numerical content. This ensures both scientific traceability and legitimate protection of sensitive assets.
