...
% --- Listes pour stocker les auteurs et leurs affiliations ---
\newcommand{\authorslist}{}
\newcommand{\affiliationslist}{}

% --- Macro pour ajouter un auteur (format : Auth<i> : Nom, Prénoms) ---
\newcommand{\addauthor}[2]{%
    \stepcounter{nid}%
    \ifnum\value{nid}=1
        \xdef\authorslist{\textbf{Auth\roman{nid} : #1, #2}}%
        \xdef\authorslistnames{#2 #1}%
    \else
        \xdef\authorslist{\authorslist, \textbf{Auth\roman{nid} : #1, #2}}%
        \xdef\authorslistnames{\authorslistnames, #2 #1}%
    \fi
}
...
\newcommand{\addaffiliation}[1]{%
    \stepcounter{nrecord}%
    \expandafter\xdef\csname affil\roman{nrecord}\endcsname{#1}%
}
...
\newcommand{\setaffiliations}[1]{%
    \expandafter\xdef\csname auth\roman{nid}affils\endcsname{#1}%
}

% --- Macro pour lister les auteurs (format avec labels Auth<i>) ---
\newcommand{\listAuthors}{\authorslist}

% --- Macro pour lister uniquement les noms des auteurs (sans labels) ---
\newcommand{\authorslistnames}{}
\newcommand{\addauthorname}[2]{%
    \ifnum\value{nid}=1
        \xdef\authorslistnames{#2 #1}%
    \else
        \xdef\authorslistnames{\authorslistnames, #2 #1}%
...
\newcommand{\listAuthorNames}{\authorslistnames}

% --- Macro pour lister toutes les affiliations ---
\newcommand{\listAffiliations}{%
    \def\tmplist{}%
    \foreach \i in {1,...,\value{nrecord}} {%
        \ifcsname affil\romannumeral\i\endcsname%
            \xdef\tmplist{\tmplist\csname affil\romannumeral\i\endcsname\\}%
        \fi%
    }%
    \tmplist%
}
...
\newcommand{\listAuthorAffiliations}[1]{%
    \edef\affilids{\csname auth#1affils\endcsname}%
    \def\tmplist{}%
    \foreach \id in \affilids {%
        \xdef\tmplist{\tmplist\csname affil\romannumeral\id\endcsname\\}%
    }%
    \tmplist%
}
...
\newcommand{\listAllAuthorsWithAffiliations}{%
    \foreach \i in {1,...,\value{nid}} {%
        \textbf{Auth\romannumeral\i} : %
        \edef\affilids{\csname auth\romannumeral\i affils\endcsname}%
        \foreach \id [count=\n] in \affilids {%
            \ifnum\n>1, \fi%
            \csname affil\romannumeral\id\endcsname%
        }%
        \\[0.2cm]%
    }%
}
