...
\begin{enumerate}
    ...
    Upon project load, \texttt{ExploreData.get\_instance()} is called, constructing a \texttt{HydrologicalTwin} instance with the project configuration. The frontend then drives the backend through the canonical macro API:
    \begin{verbatim}
        twin.configure(mode="private", key="user_key.pem")  # Private access
        twin.load(kind="project_state")
    \end{verbatim}
    ...
    \end{verbatim}
    Dialogs call only the canonical \texttt{HydrologicalTwin} macro methods (e.g., \texttt{describe}, \texttt{extract}, \texttt{transform}, \texttt{render}).
    ...
    \begin{itemize}
        ...
        \begin{verbatim}
            data = twin.extract(kind="spatial_map", request=request)
        \end{verbatim}
        \item \textbf{Private operations} (e.g., raw data extraction):
        \begin{verbatim}
            data = twin.extract(kind="timeseries", request=request)  # Requires private-key
        \end{verbatim}
    \end{itemize}
    Services perform:
    \begin{itemize}
